...
function PMA_DBI_try_query($query, $link = null, $options = 0) {
    ...
    if ($options == ($options | PMA_DBI_QUERY_STORE)) {
        return @mysqli_query($link, $query, $link);
...
//            return FALSE;
    ...
    if (null !== $link) {
        ...
        $error_message = mysqli_error($link, $link);
    } else {
    ...
    // Some errors messages cannot be obtained by mysqli_error($link)
...
function PMA_DBI_get_fields_meta($result) {
    ...
    for ($i = 0; $i < $num_fields; $i++) {
        $fields[] = PMA_convert_display_charset(mysqli_fetch_field($result, $i));
    }
    return $fields;
}
