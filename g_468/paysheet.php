...
<html>
<head>
<title>CANTERBURY -- paysheet</title>
...
<body>
<table class="bd" width="100%"><tr><td class="hr"><h2>PAY SHEET</h2></td></tr></table>
<?php $conn = connect();
  $showrecs = 20;
  $pagerange = 10;
...
<td><a href="paysheet.php?a=reset">Reset Filter</a></td>
</tr>
</table>
</form>
<hr size="1" noshade>
...
<?php reset($fields);
  foreach($fields as $val => $caption) {
?>
<td class="hr"><a class="hr" href="paysheet.php?order=<?php echo $val ?>&type=<?php echo $ordtypestr ?>"><?php echo htmlspecialchars($caption) ?></a></td>
<?php } ?>
<td class="hr">&nbsp;</td>
<td class="hr">&nbsp;</td>
<td class="hr">&nbsp;</td>
...
<?php } ?>
<td class="<?php echo $style ?>"><a href="paysheet.php?a=view&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">View</a></td>
<td class="<?php echo $style ?>"><a href="paysheet.php?a=edit&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">Edit</a></td>
<td class="<?php echo $style ?>"><a href="paysheet.php?a=del&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">Delete</a></td>
</tr>
<?php }
...
<tr>
<td class="hr"><?php echo htmlspecialchars("changes")."&nbsp;" ?></td>
<td class="dr"><?php echo htmlspecialchars($row["changes"]) ?></td>
</tr>
<tr>
<td class="hr"><?php echo htmlspecialchars("total")."&nbsp;" ?></td>
<td class="dr"><?php echo htmlspecialchars($row["total"]) ?></td>
</tr>
</table>
<?php } ?>
...
<tr>
<td class="hr"><?php echo htmlspecialchars("changes")."&nbsp;" ?></td>
<td class="dr"><input type="text" name="changes" value="<?php echo str_replace('"', '&quot;', trim($row["changes"])) ?>"></td>
</tr>
<tr>
<td class="hr"><?php echo htmlspecialchars("total")."&nbsp;" ?></td>
<td class="dr"><input type="text" name="total" value="<?php echo str_replace('"', '&quot;', trim($row["total"])) ?>"></td>
</tr>
</table>
<?php } ?>
...
<table class="bd" border="0" cellspacing="1" cellpadding="4">
<tr>
<td><a href="paysheet.php?a=add">Add Record</a>&nbsp;</td>
<?php if ($page > 1) { ?>
<td><a href="paysheet.php?page=<?php echo $page - 1 ?>">&lt;&lt;&nbsp;Prev</a>&nbsp;</td>
<?php } ?>
...
<?php } else { ?>
<td><a href="paysheet.php?page=<?php echo $j ?>"><?php echo $j ?></a></td>
<?php } } } else { ?>
<td><a href="paysheet.php?page=<?php echo $startpage ?>"><?php echo $startpage ."..." .$count ?></a></td>
<?php } } } ?>
<?php if ($page < $pagecount) { ?>
<td>&nbsp;<a href="paysheet.php?page=<?php echo $page + 1 ?>">Next&nbsp;&gt;&gt;</a>&nbsp;</td>
<?php } ?>
</tr>
</table>
<?php } ?>
...
<table class="bd" border="0" cellspacing="1" cellpadding="4">
<tr>
<td><a href="paysheet.php">Index Page</a></td>
<?php if ($recid > 0) { ?>
<td><a href="paysheet.php?a=<?php echo $a ?>&recid=<?php echo $recid - 1 ?>">Prior Record</a></td>
<?php } if ($recid < $count) { ?>
<td><a href="paysheet.php?a=<?php echo $a ?>&recid=<?php echo $recid + 1 ?>">Next Record</a></td>
<?php } ?>
</tr>
</table>
<hr size="1" noshade>
...
<tr>
<td><a href="paysheet.php?a=add">Add Record</a></td>
<td><a href="paysheet.php?a=edit&recid=<?php echo $recid ?>">Edit Record</a></td>
<td><a href="paysheet.php?a=del&recid=<?php echo $recid ?>">Delete Record</a></td>
</tr>
</table>
...
function sql_select()
{
  ...
  $sql = "select `id`, `user_id`, `day`, `observations`, `changes`, `total` from `paysheet`";
  if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    $sql .= " where " .sqlstr($filterfield) ." like '" .$filterstr ."'";
  } elseif (isset($filterstr) && $filterstr!='') {
    $sql .= " where (`id` like '" .$filterstr ."') or (`user_id` like '" .$filterstr ."') or (`day` like '" .$filterstr ."') or (`observations` like '" .$filterstr ."') or (`changes` like '" .$filterstr ."') or (`total` like '" .$filterstr ."')";
  }
...
function sql_getrecordcount()
{
  ...
  $filterstr = sqlstr($filter);
  if (!$wholeonly && isset($wholeonly) && $filterstr!='') $filterstr = "%" .$filterstr ."%";
  $sql = "select count(*) from `paysheet`";
  if (isset($filterstr) && $filterstr!='' && isset($filterfield) && $filterfield!='') {
    $sql .= " where " .sqlstr($filterfield) ." like '" .$filterstr ."'";
  } elseif (isset($filterstr) && $filterstr!='') {
    $sql .= " where (`id` like '" .$filterstr ."') or (`user_id` like '" .$filterstr ."') or (`day` like '" .$filterstr ."') or (`observations` like '" .$filterstr ."') or (`changes` like '" .$filterstr ."') or (`total` like '" .$filterstr ."')";
...
function sql_insert()
{
  ...
  $sql = "insert into `paysheet` (`id`, `user_id`, `day`, `observations`, `changes`, `total`) values (" .sqlvalue(@$_POST["id"], false) .", " .sqlvalue(@$_POST["user_id"], false) .", " .sqlvalue(@$_POST["day"], true) .", " .sqlvalue(@$_POST["observations"], true) .", " .sqlvalue(@$_POST["changes"], false) .", " .sqlvalue(@$_POST["total"], false) .")";
...
function sql_update()
{
  ...
  $sql = "update `paysheet` set `id`=" .sqlvalue(@$_POST["id"], false) .", `user_id`=" .sqlvalue(@$_POST["user_id"], false) .", `day`=" .sqlvalue(@$_POST["day"], true) .", `observations`=" .sqlvalue(@$_POST["observations"], true) .", `changes`=" .sqlvalue(@$_POST["changes"], false) .", `total`=" .sqlvalue(@$_POST["total"], false) ." where " ."(`id`=" .sqlvalue(@$_POST["xid"], false) .")";
  mysqli_query($link, $sql, $conn) or die(mysqli_error($link));
}
...
function sql_delete()
{
  ...
  $sql = "delete from `paysheet` where " ."(`id`=" .sqlvalue(@$_POST["xid"], false) .")";
  mysqli_query($link, $sql, $conn) or die(mysqli_error($link));
} ?>
...
?>
<br>
<form action="paysheet.php<?php echo $page?>" method="post">
<input type="hidden" name="sql" value="update">
<input type="hidden" name="xid" value="<?php echo $row["id"] ?>">
<?php showroweditor($row) ?>
<p><input type="submit" name="action" value="Post"></p>
...
?>
<br>
<form action="paysheet.php?page=<?php echo $page?>" method="post">
<input type="hidden" name="sql" value="delete">
<input type="hidden" name="xid" value="<?php echo $row["id"] ?>">
<?php showrow($row) ?>
<p><input type="submit" name="action" value="Confirm"></p>
...
<?php echo DameSelects('select id,id,name from users ORDER BY name');		
?>
</select></td>
</tr>
<tr>
<td class="hr"><?php echo htmlspecialchars("day (yyyy-dd-mm)")."&nbsp;" ?></td>
<td class="dr"><input type="text" name="day" value="<?php echo str_replace('"', '&quot;', trim($row["day"])) ?>"></td>
</tr>
<tr>
<td class="hr"><?php echo htmlspecialchars("observations")."&nbsp;" ?></td>
...
<?php //######################################################################
 function DameUltimoMasUno()
	 ...
	 global $conn;
//////////////////////////////
			$sql = 'SELECT id FROM paysheet ORDER BY id desc'; //where ".$searchtype." like '%".$searchterm."%'";
   		    
			//print 'cccccccccccccccccccccccccc';
			
  			$rs = mysqli_query($link, $sql, $conn);//mysqli_query($link, $sql);
 ...
 function DameUltimoMasUno2()
	 ...
	 global $conn;
//////////////////////////////
			$sql = 'SELECT old_id FROM paysheet ORDER BY id desc'; //where ".$searchtype." like '%".$searchterm."%'";
