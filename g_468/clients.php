...
<body>
<table class="bd" width="100%"><tr><td class="hr"><h2>GESTIÓN ALUMNOS DE EMPRESAS</h2></td></tr></table>
<?php
  $conn = connect();
  $showrecs = 20;
  $pagerange = 10;
...
<?php } ?>
<td class="<?php echo $style ?>"><a href="clients.php?a=view&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">View</a></td>
<td class="<?php echo $style ?>"><a href="clients.php?a=edit&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">Edit</a></td>
<td class="<?php echo $style ?>"><a href="clients.php?a=del&recid=<?php echo $i ?>&id=<?php echo $row['id'] ?>">Delete</a></td>
</tr>
<?php
  }
...
function sql_selectJBT($parId)
{
  ...
  //if (isset($order) && $order!='') $sql .= " order by \"" .sqlstr($order) ."\"";
  //if (isset($ordtype) && $ordtype!='') $sql .= " " .sqlstr($ordtype);
  $sql = "select `id`, `old_id`, `group_id`, `name`, `telephone1`, `telephone2`, `mobile`, `fax`, `email`, `address1`, `address2`, `city`, `state`, `zip` from `clients` where id=".$parId;
...
//######################################################################
 function DameUltimoMasUno()
	 ...
	 global $conn;
//////////////////////////////
			$sql = 'SELECT id FROM clients ORDER BY id desc'; //where ".$searchtype." like '%".$searchterm."%'";
 ...
 function DameUltimoMasUno2()
	 ...
	 global $conn;
//////////////////////////////
			$sql = 'SELECT old_id FROM clients ORDER BY id desc'; //where ".$searchtype." like '%".$searchterm."%'";
...
<?php
 echo DameSelects('select id,old_id,name from groups ORDER BY name');		
?>
</select></td>
</tr>
<tr>
