...
<input name="selected_day" type="hidden" id="selected_day" value="<?php echo($sld);?>" />
    ...
    <div id="calendar-container">
        ...
        <table border="0" cellspacing="1" cellpadding="3" align="center" class="bg">
			...
			$lista = $objblockdays->consultar();
			$fechaBD = array();
			$j = 0;
			while($rowBlock = mysqli_fetch_array($lista)){
			 //echo $day."DAY";
	            //echo $y."-".$m."-".$day."FECHA";
				//echo $rowBlock["fechas"]."F";
				$fechaBD[$j] = $rowBlock["fechas"];
				$j++;
			
		     }
		//fin-raftmodification
            //write current month
	       ...
	       // echo $fechass[$day]; 
			 for($i=0; $i<count($fechaBD); $i++){
			
				//echo $fechaBD[$i]."NO";
			  if($fechass[$day] == $fechaBD[$i]){
			  $dateLink = false;
