...
Sigma.Ajax = Sigma.$class( {
	...
	send: function(options){
		...
		if ( method == 'post'){
			// todo :ã€€fixed url too long . err code : 122
			var idx= url.indexOf('?');
			if (idx>=0){
...
Sigma.DataSetDefault = {
	...
	properties : function(){return {
		...
		onInsertRecord : Sigma.$empty,
		onUpdateRecord : Sigma.$empty
...
/* todo */
	...
	insertRecord : function(record){
		...
		Sigma.$invoke(this,'onInsertRecord',[record]);
		this.modified=true;
	},
...
/* todo */
	updateRecord : function(record,fieldName, newValue){
		...
		if (this.insertedRecords[ sn  ] || Sigma.$invoke(this,'onUpdateRecord',[record,fieldName, newValue])!==false){
			if (type=='int') {
...
////
/*
1. following api need to be done
2. filter with paging
3. ie8
4. auto height  
5. checkbox issue all  

*/

//----the following API need to be implemented--------------.
/*
 *
 * 
 ...
 * grid show/hide 
 * 
 * ^set/getDimension set/getSkin 
 * 
 * ^header setHeader
 * 
 * Paging set/getPageInfo gotoPage 
 *  
 * ^showDialog("filter"/"chart") hideDialog; 
 * 
 * ^Row ^addRow ^deleteRow ^selectRow
 * updateRow checkRow set/getRowStyle
 * 
 * 
 * Column 
 * ^MoveTo ^set/getWidth ^sortGrid ^Groupby
 * 
 * Cell 
 * get/setFocusCell get/setEditingCell 
 * 
 * Grid
 * 
 */
 ...
 * * Grid hide/showRowNumber set/getRowStriple 
 * * Row moveTo hide/show Copy/Paste
 * * record addFields getFields
 * * editor set/getCurrentEditor this can be implemented by onGetEditor
 */
...
/////////////////////
	...
	properties : function(){return {
		...
		skinList : [
			...
			{ text:this.getMsg('STYLE_NAME_CHINA'),value:'china'},
	...
	////////////////////////////////////
        ...
        /**
	 	 * @description {Event} beforeSelectRow:function(record,row, rowNo,grid)
		...
		/**
	 	 * @description {Event} afterSelectRow:function(value, record , cell, row, colNO, rowNO, columnObj,grid)
		...
		/**
	 	 * @description {Event} onClickHead:function(event,headCell,colObj,grid)
		...
		/**
	 	 * @description {Event} onClickCell:function(value, record , cell, row, colNO, rowNO,columnObj,grid)
        ...
        /**
	 	 * @description {Event} onDblClickCell:function(value, record, cell, row, colNO, rowNO,columnObj,grid)
		...
		/**
	 	 * @description {Event} beforeEdit:function() Fired before cell is open for editing. 
		...
		/**
	 	 * @description {Event} afterEdit:function() Fired before cell is out of editable mode. 
	 	 * Developer can get active cell by refering to activeCell. See Sigma.Grid.activeCell. 
	 	 * Developer can get active record by refering to activeRecord. See Sigma.Grid.activeRecord        
	     */
		...
		beforeUpdateDataset : Sigma.$empty ,
		...
		/**
	 	 ...
	 	 * @param {Boolean} Return false to prevent row from being added.          
	     */
		...
		/**
	 	 ...
	 	 * @param {Boolean} Return false to prevent record to be updated.          
	     */
		...
		/**
	 	 ...
	 	 * @param {Boolean} Return false to prevent row from being removed.          
	     */
		beforeDelete : Sigma.$empty,
		/**
	 	 * @description {Event} afterDelete Changed data is to be posted to server side.
	     * @param {Object or Array} record Record of the row added/appended by end user.
	     * @return {Boolean} Return false to prevent row from being posted to server side.
	     */
	...
	initActiveObj : function(event , newActiveCell){
			...
			if (newActiveCell && !this.isGroupRow(newActiveCell) ) {
				this.closeGridMenu();
				var _change=newActiveCell!=this.activeCell;
				if (_change){
					Sigma.U.removeClass(this.activeCell,'gt-cell-actived'+( this.activeEditor?'-editable':''));
					this.syncTwinRowCell(null,this.activeCell);
				}

				this.syncActiveObj(newActiveCell);
...
/*
		...
		grid.onmouseover = grid.onmouseover || grid.onMouseOver;
		if (grid.onmouseover) {
			Sigma.U.addEvent(grid.bodyDiv,"mouseover",function(event){
				Sigma.$invoke(grid,'onmouseover',[event,grid])
			}) ;
		}
		grid.onmouseout = grid.onmouseout || grid.onMouseOut;
		if (grid.onmouseout) {
			Sigma.U.addEvent(grid.bodyDiv,"mouseout",function(event){
				Sigma.$invoke(grid,'onmouseout',[event,grid])
			}) ;
		}

		grid.onmousemove = grid.onmousemove || grid.onMouseMove;
		if (grid.lightOverRow) {
			Sigma.U.addEvent(grid.bodyDiv,"mousemove",function(event){
				Sigma.Grid.handleOverRow(event,grid);
				Sigma.$invoke(grid,'onmousemove',[event,grid,grid.bodyDiv]);
			}) ;

			Sigma.U.addEvent(grid.freezeBodyDiv,"mousemove",function(event){
				Sigma.Grid.handleOverRowF(event,grid);
				Sigma.$invoke(grid,'onmousemove',[event,grid,grid.freezeBodyDiv]);
			}) ;
		}
*/
	clickCount : 0,
...
//load, changePage
	...
	/**
	 * @description {Method} exportGrid To export grid to a file. This function need server side programming supported.
     ...
     * @return {Sigma.Column}Column object.
     */
     ...
     * @param {Integer} recordOrRowNo  Number of row where cell is at. 
	...
	/**
	 * @description {Method} printGrid To print grid. This function will popup a printer-choosing dialog box. 
	 */
	printGrid : function(){
			var grid=this ,docT;
			...
			grid.showWaiting();
				...
				var printCssText=[
					...
					'.gt-table TD { font-size:14px;padding:2px; border-right:1px solid #000000 ; border-bottom:1px solid #000000; }' ,
...
month[11]="November";
month[12]="December";
	var rowtest = this.getColumnValue("hide",0);
				var res = rowtest.split("|");
  ...
  var index =  parseInt(date[1]);
 			
  var n = month[index];
	  		
				var printCssText1=['<b>'+n+' '+date[0]+"&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;&emsp;PAYSHEET</b><br><br>"];
				...
				var totaleuros = this.getColumnValue("total",0);
				var totalclasses = this.getColumnValue("size",0);
				var totalfechas = this.getColumnValue("taught",0);
				var lsc = this.getColumnValue("late_student",0);
				var mk = this.getColumnValue("makeup_classes",0);
				
			
				var printCssText2=[
					'<b><br><font size="5">Teacher:  </font><font size="5" color="green">'+res[0]+'</font><br><br>Total Euros: <font size="4" color="green">'+totaleuros+' €</font>&emsp;&emsp;&emsp;Total Class: <font size="4" color="green">'+totalclasses+'</font>&emsp;&emsp;&emsp;Class Taught: <font size="4" color="green">'+totalfechas+'</font>&emsp;&emsp;&emsp;Late Student Cancellation: <font size="4" color="red">'+lsc+'</font>&emsp;&emsp;&emsp;Makeup Taught: <font size="4" color="green">'+mk+'</font><br><br><br>Date:_________________________________&emsp;Signature(Sign only after you have received payment):______________________<br><br><font size="4">OBSERVATIONS:' +ob1+'<br> '+ob2+'<br> '+ob3+'<br> '+ob4+'<br> '+ob5+'<br> '+ob6+'<br> '+ob7+'<br> '+ob8+' </font></b>'
				];
			...
			function printBody(_doc) {
				...
				_doc.writeln( '</style>' );
				_doc.writeln( printCssText1 );
				_doc.writeln(gridHTML);
				_doc.writeln( printCssText2 );
				_doc.close();
			}
	...
	save : function(onNav){
		...
		/*if (!onNav && !hasModified) {
			...
			//rafa var validResult=this.activeEditor.doValid(value,this.activeRecord,this.activeColumn,this);
			var rows= this.gridTable.tFoot ? this.gridTable.tFoot.rows : [];
			
			//for (var i=0,len=12;i<len ;i++) {
				var _ir;
				//alert(this.columnList.length+"tttt");
				for (var cn=0;cn<this.columnList.length ;cn++ )	{
					//alert(rows+"HHH");
					var colObj=this.columnList[cn];
			...
			//alert(this.dataset.fieldsName+"NNNN");
	
				/*alert(Sigma.Const.DataSet.KEY+"FFFF");
			alert(record[0]+"NNNN");
		  alert(record[ Sigma.Const.DataSet.SN_FIELD ]+"BBBB");*/
	...
	/**
     * @description {Method} showMask To show a mask.
     * @param {Object} trp
     */ 
	...
	applyFilter : function(filterInfo){
		...
		if (this.tools.filterTool) {
			if (this.filterInfo.length>0){
			    ...
			    Sigma.U.removeClass(this.filterTool.itemIcon,"gt-tool-filtered");
		    }
		}
     ...
     * @param {Object} newWidth New width.
     * @param {Object} newHeight New height.
     * @param {Object} isInit
     */ 
	 ...
	 * @param {Function} fn row: The object of the row. record: The data record behine the row. i: Loop counter. grid: Grid object.
	 */
	 ...
	 * @param {Integer} oldIndex Index of column to be moved.
	 * @param {Integer} newIndex Index of new position.
...
/**
  ...
  *
  * @classDescription   This class creates a new Grid.
  * @alias Sigma.Grid
  * @inherits Sigma.Grid
  * @return {Shape} Returns a new Grid.
  * @type {Object}
  * @constructor    
  */
Sigma.$extend( Sigma.Grid , {
	...
	endDocGridHandler : function(evt,grid){
		...
		if (grid.separateLine && grid.separateLine.style.display=="block"){
			...
			/**
	 		 * @description {Event} Fired after one column is resized.
...
//		
		...
		Sigma.U.addEvent( d, 'keydown' ,function(event){
			Sigma.activeGrid && Sigma.activeGrid._onKeydown(event); 
		});
...
/* todo */
    ...
    /**
     * To set column width.
     * @param {Integer} newWidth Column new width in pixel.
     */
	 ...
	 * @param {Object} header Header inner html
	 */
...
/**
  ...
  * @classDescription   This class creates a new Shape.
  * @alias Sigma.Column
  * @inherits Sigma.Column
  * @return {Shape} Returns a new column.
  * @type {Object}
  * @constructor    
  */
...
Sigma.checkChecked = function(grid){
	...
	var htd=Sigma.U.getParentByTagName('td',chkAll); //È¡ï¿½ï¿½ï¿½ï¿½checkBoxï¿½ï¿½ï¿½Úµï¿½td

	var cellIdx=Sigma.U.getCellIndex(htd);	//È¡ï¿½ï¿½tdï¿½ï¿½ï¿½ï¿½ï¿½ï¿½(ï¿½ÇµÚ¼ï¿½ï¿½ï¿½)
	...
	for (var i=0,j=rows.length;i<j; i++ ){
		...
		if (cell){
			var _chk=cell.getElementsByTagName('input')[0];	//È¡ï¿½Ã¶ï¿½Ó¦ï¿½ï¿½Ôªï¿½ï¿½ï¿½Úµï¿½ checkbox
...
Sigma.createFilterDialog = function(cfg){
	...
	var okFn= function(){
		...
		if (filterInfo.length>0){
			// ï¿½ï¿½ï¿½Ø·ï¿½Ï¹ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½ï¿½Ðºï¿½(ï¿½ï¿½ï¿½ï¿½);
