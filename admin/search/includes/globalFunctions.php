...
///////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	function DameOpcionesStudents($IdStudent, $idalumno) {
  			...
  			$sql = "select a.id as ident, a.name FROM clients a where a.group_id = '" . $IdStudent .  "' order by a.name";
	...
	if ($numfilas <= 0) {
		...
		while ($row = mysqli_fetch_array($result, MYSQLI_ASSOC)) {
			...
			$alumno = $row["name"];
			$opciones1 = $id.",".$alumno;
			$students = $students. "<option value='" .$opciones1. "'" . DameSeleccionCnf($idalumno,$row['ident']) . ">" . $row["name"]. "</option>\n";  			
	   	}  
	}
			return $students;
	} 
	...
	$datereport = date(Y)."-".$month."-01";
	 
	 		if ($teacher == "admin"){
		  
			$sql = "SELECT a.id as ident, a.name, a.email, b.end, b.id, class_type, length FROM groups a, userGroup b where b.group_id = a.id and (b.end is null or b.end >= '".$datereport."') order by name";
			
		} else {
  			$sql = "select a.end, b.id, a.name, a.email, class_type, length, a.id as ident from groups a, userGroup b, users c ";
