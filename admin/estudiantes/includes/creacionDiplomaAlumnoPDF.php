...
<?php
  ...
  ////////////////////////////////////////////////////////////////////////////////////////////////////////////////  
	...
	return $resul; 
			...
			function DameGroupId($id) { 
     global $Conexion;
	  // echo $client_id1."XXXX";
	$sql='SELECT IDCLIENTE FROM informes_alumnos2 WHERE ID='.$id;
	 // echo $sql."XXXX";
	 $rs = mysqli_query($link, $sql) or die("Invalid query: " . mysqli_error($link));	
	if ($fila = mysqli_fetch_row($rs, MYSQLI_ASSOC)){				
	
	    $resul=$fila["IDCLIENTE"];
		}
	 else {	
	   $resul='';
	   }	
...
function DameTotalHoras($id) { 
    $group_id = DameGroupId($id);
	
	 $sql='SELECT sum( hours ) as sum FROM entry WHERE group_id ='.$group_id;
//          echo $sql;
	 $rs = mysqli_query($link, $sql) or die("Invalid query: " . mysqli_error($link));	
	if ($fila = mysqli_fetch_row($rs, MYSQLI_ASSOC)){				
...
//	$id=$_REQUEST['id'];
	...
	//	require("pdf_lib/fpdf.php");
		...
		if ($TipoInforme==1){
		$pdf->Cell(14,1.2,iconv('UTF-8', 'windows-1252', $Alumno),0,1,C);}
		else{
		$pdf->Cell(14.5,0.9,iconv('UTF-8', 'windows-1252', $Alumno),0,0,C);
		}
